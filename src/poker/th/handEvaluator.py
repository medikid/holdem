...
#class self.HandLengthException(Exception):
#    pass
...
class HandEvaluator:
        ...
        def evaluate_percentile(self, hand):
            ...
            if hand[0].suiteRank == hand[1].suiteRank:
                if hand[0].cardRank < hand[1].cardRank:
                    return LookupTables.Two.suited_ranks_to_percentile[hand[0].cardRank][hand[1].cardRank]
                else:
                    return LookupTables.Two.suited_ranks_to_percentile[hand[1].cardRank][hand[0].cardRank]
            else:
                return LookupTables.Two.unsuited_ranks_to_percentile[hand[0].cardRank][hand[1].cardRank]
    ...
    class Five:
        def card_to_binary(self, card):
            ...
            b_mask = 1 << (14 + card.cardRank)
            cdhs_mask = 1 << (card.suiteRank + 11)
            r_mask = (card.cardRank - 2) << 8
            p_mask = LookupTables.primes[card.cardRank - 2]
            # OR them together to get the final result
            return b_mask | r_mask | p_mask | cdhs_mask

        def card_to_binary_lookup(self, card):
            return LookupTables.Five.card_to_binary[card.cardRank][card.suiteRank]
        ...
        def evaluate_rank(self, hand):
            ...
            """
            if len(hand) != 5:
                raise self.HandLengthException("Only 5-card hands are supported by the Five evaluator")
    ...
    class Six:
        def card_to_binary(self, card):
            ...
            # Then shift appropriately to fit the template above
            b_mask = 1 << (14 + card.cardRank)
            q_mask = LookupTables.primes[card.suiteRank - 1] << 12
            r_mask = (card.cardRank - 2) << 8
            p_mask = LookupTables.primes[card.cardRank - 2]
            # OR them together to get the final result
        ...
        def card_to_binary_lookup(self, card):
            return LookupTables.Six.card_to_binary[card.cardRank][card.suiteRank]
    ...
    class Seven:
        ...
        def card_to_binary_lookup(self, card):
            return LookupTables.Seven.card_to_binary[card.cardRank][card.suiteRank]
    ...
    def evaluate_hand(self, hand, board=[]):
        ...
        if len(hand) not in hand_lengths:
            raise self.HandLengthException("Only %s hole cards are supported" % ", ".join(map(str, hand_lengths)))
        ...
        if len(cards) == 2:
            ...
            raise self.HandLengthException("Only 2, 5, 6, 7 cards total are supported by evaluate_hand")
